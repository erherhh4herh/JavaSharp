...
namespace java.security
{
	...
	/// <para>This would allow somebody to introduce a possibly
	/// malicious provider (e.g., one that discloses the private keys passed
	/// to it) as the highest-priority provider. This would be possible
	/// because the Security object (which manages the installed providers)
	/// currently does not check the integrity or authenticity of a provider
	...
	public sealed class SecurityPermission : BasicPermission
	{
		...
		private const long SerialVersionUID = 5236109936224050470L;
		...
		public SecurityPermission(String name) : base(name)
		{
		}
		...
		/// <exception cref="NullPointerException"> if {@code name} is {@code null}. </exception>
		/// <exception cref="IllegalArgumentException"> if {@code name} is empty. </exception>
		public SecurityPermission(String name, String actions) : base(name, actions)
		{
		}
	}
