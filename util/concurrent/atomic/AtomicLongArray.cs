...
namespace java.util.concurrent.atomic
{
	...
	{
		private const long SerialVersionUID = -2308431214976778248L;

		private static readonly Unsafe @unsafe = Unsafe.Unsafe;
		private static readonly int @base = @unsafe.arrayBaseOffset(typeof(long[]));
		private static readonly int Shift;
		private readonly long[] Array;

		static AtomicLongArray()
		{
			int scale = @unsafe.arrayIndexScale(typeof(long[]));
			if ((scale & (scale - 1)) != 0)
			{
		...
		private bool CompareAndSetRaw(long offset, long expect, long update)
		{
