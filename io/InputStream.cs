...
namespace java.io
{
	...
	/// 
	/// <para> Applications that need to define a subclass of <code>InputStream</code>
	...
	public abstract class InputStream : Closeable
	{
		...
		/// 
		/// <para> A subclass must provide an implementation of this method.
		/// 
		/// </para>
		/// </summary>
...
//ORIGINAL LINE: public abstract int read() throws IOException;
		...
		/// 
		/// <para> If the length of <code>b</code> is zero, then no bytes are read and
		...
		/// 
		/// </para>
		/// <para> The <code>read(b)</code> method for class <code>InputStream</code>
...
//ORIGINAL LINE: public int read(byte b[]) throws IOException
		...
		/// 
		/// </para>
		/// <para> The <code>read(b,</code> <code>off,</code> <code>len)</code> method
...
//ORIGINAL LINE: public int read(byte b[] , int off, int len) throws IOException
		...
		/// value differently.
		/// 
		/// <para> The <code>skip</code> method of this class creates a
...
//ORIGINAL LINE: public long skip(long n) throws IOException
		public virtual long Skip(long n)
		{
			...
			int size = (int)System.Math.Min(MAX_SKIP_BUFFER_SIZE, remaining);
			sbyte[] skipBuffer = new sbyte[size];
			while (remaining > 0)
			{
				nr = Read(skipBuffer, 0, (int)System.Math.Min(size, remaining));
				if (nr < 0)
				{
					break;
				}
		...
		/// 
		/// </para>
		/// <para> A subclass' implementation of this method may choose to throw an
		/// <seealso cref="IOException"/> if this input stream has been closed by
		/// invoking the <seealso cref="#close()"/> method.
		/// 
		/// </para>
		/// <para> The {@code available} method for class {@code InputStream} always
		/// returns {@code 0}.
		/// 
		/// </para>
...
//ORIGINAL LINE: public int available() throws IOException
		...
		/// with the stream.
		/// 
		/// <para> The <code>close</code> method of <code>InputStream</code> does
		/// nothing.
		/// 
		/// </para>
		/// </summary>
...
//ORIGINAL LINE: public void close() throws IOException
		...
		/// 
		/// </para>
		/// <para> Marking a closed stream should not have any effect on the stream.
		/// 
		/// </para>
