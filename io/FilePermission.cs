...
namespace java.io
{
	...
	/// 
	/// <para>Please note: Code can always read a file from the same
	...
	public sealed class FilePermission : Permission
	{
		...
		/// Read action.
		/// </summary>
		private const int READ = 0x4;
		/// <summary>
		/// Delete action.
		/// </summary>
		private const int DELETE = 0x8;
		/// <summary>
		/// Read link action.
		/// </summary>
		private const int READLINK = 0x10;

		/// <summary>
		/// All actions (read,write,execute,delete,readlink)
		/// </summary>
		private static readonly int ALL = READ | WRITE | EXECUTE | DELETE | READLINK;
		/// <summary>
		/// No actions.
		/// </summary>
		...
		private const char RECURSIVE_CHAR = '-';
		private const char WILD_CHAR = '*';
	...
	*/
		...
		private void Init(int mask)
		{
			...
			Cpath = AccessController.doPrivileged(new PrivilegedActionAnonymousInnerClassHelper(this));

			int len = Cpath.Length();
			char last = ((len > 0) ? Cpath.CharAt(len - 1) : 0);

			if (last == RECURSIVE_CHAR && Cpath.CharAt(len - 2) == System.IO.Path.DirectorySeparatorChar)
			{
				Directory = true;
				Recursive = true;
				Cpath = Cpath.Substring(0, --len);
			}
			else if (last == WILD_CHAR && Cpath.CharAt(len - 2) == System.IO.Path.DirectorySeparatorChar)
			{
				Directory = true;
				//recursive = false;
				Cpath = Cpath.Substring(0, --len);
		...
		{
			private readonly FilePermission OuterInstance;
			...
			public virtual String Run()
			{
				try
				{
					String path = OuterInstance.Cpath;
					if (OuterInstance.Cpath.EndsWith("*"))
					{
		...
		/// 
		/// <para>A pathname that ends in "/*" (where "/" is
		...
		/// 
		/// </para>
		/// <para>A pathname consisting of a single "*" indicates all the files
		/// in the current directory, while a pathname consisting of a single "-"
		/// indicates all the files in the current directory and
		/// (recursively) all files and subdirectories contained in the current
		/// directory.
		/// 
		/// </para>
		/// <para>A pathname containing an empty string represents an empty path.
		/// 
		/// </para>
		/// </summary>
		...
		public FilePermission(String path, String actions) : base(path)
		{
		...
		internal FilePermission(String path, int mask) : base(path)
		{
			Init(mask);
		}
		...
		/// 
		/// </para>
		/// <para>For example, if you have two FilePermissions:
		/// <OL>
		...
		/// 
		/// </para>
		/// <para>and you are calling the <code>implies</code> method with the FilePermission:
		/// 
		/// <pre>
...
//ORIGINAL LINE: private void readObject(ObjectInputStream s) throws IOException, ClassNotFoundException
	...
	[Serializable]
	internal sealed class FilePermissionCollection : PermissionCollection
	{
		// Not serialized; see serialization section at end of class
		[NonSerialized]
		private IList<Permission> Perms;
