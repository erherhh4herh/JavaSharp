...
namespace java.awt.image
{
	...
	public class DirectColorModel : PackedColorModel
	{
		...
		public DirectColorModel(int bits, int rmask, int gmask, int bmask, int amask) : base(ColorSpace.GetInstance(ColorSpace.CS_sRGB_Renamed), bits, rmask, gmask, bmask, amask, false, amask == 0 ? java.awt.Transparency_Fields.OPAQUE : java.awt.Transparency_Fields.TRANSLUCENT, ColorModel.GetDefaultTransferType(bits))
		{
			SetFields();
		}
		...
		public DirectColorModel(ColorSpace space, int bits, int rmask, int gmask, int bmask, int amask, bool isAlphaPremultiplied, int transferType) : base(space, bits, rmask, gmask, bmask, amask, isAlphaPremultiplied, amask == 0 ? java.awt.Transparency_Fields.OPAQUE : java.awt.Transparency_Fields.TRANSLUCENT, transferType)
		{
			if (ColorModel.IsLinearRGBspace(ColorSpace_Renamed))
			{
				Is_LinearRGB = true;
				if (MaxBits <= 8)
				{
		...
		private float[] GetDefaultRGBComponents(int pixel)
		{
			...
			return ColorSpace_Renamed.ToRGB(norm);
		}


		private int GetsRGBComponentFromsRGB(int pixel, int idx)
		{
			int c = ((int)((uint)(pixel & MaskArray[idx]) >> MaskOffsets[idx]));
			if (IsAlphaPremultiplied)
			{
				int a = ((int)((uint)(pixel & MaskArray[3]) >> MaskOffsets[3]));
		...
		public override Object GetDataElements(int rgb, Object pixel)
		{
			...
			ColorModel defaultCM = ColorModel.RGBdefault;
			...
			switch (TransferType_Renamed)
			{
				case DataBuffer.TYPE_BYTE:
				{
				   ...
				   bdata[0] = unchecked((sbyte)(0xff & intpixel[0]));
				   return bdata;
				}
				case DataBuffer.TYPE_USHORT:
				{
				   ...
				   sdata[0] = unchecked((short)(intpixel[0] & 0xffff));
				   return sdata;
				}
				case DataBuffer.TYPE_INT:
				   return intpixel;
		...
		public sealed override int[] GetComponents(int pixel, int[] components, int offset)
		{
			if (components == null)
			{
			...
			{
				components[offset + i] = (int)((uint)(pixel & MaskArray[i]) >> MaskOffsets[i]);
			}
		...
		public sealed override int[] GetComponents(Object pixel, int[] components, int offset)
		{
			int intpixel = 0;
			switch (TransferType_Renamed)
			{
		...
		public sealed override WritableRaster CreateCompatibleWritableRaster(int w, int h)
		{
			if ((w <= 0) || (h <= 0))
			{
			...
			switch (TransferType_Renamed)
			{
				case DataBuffer.TYPE_BYTE:
				   if (obj is sbyte[])
				   {
					   ...
					   bdata[0] = unchecked((sbyte)(pixel & 0xff));
					   return bdata;
				   }
				   else
				   {
					   sbyte[] bdata = new sbyte[] {unchecked((sbyte)(pixel & 0xff))};
					   return bdata;
				   }
				case DataBuffer.TYPE_USHORT:
				   if (obj is short[])
				   {
					   ...
					   sdata[0] = unchecked((short)(pixel & 0xffff));
					   return sdata;
				   }
				   else
				   {
					   short[] sdata = new short[] {unchecked((short)(pixel & 0xffff))};
					   return sdata;
				   }
				case DataBuffer.TYPE_INT:
				   if (obj is int[])
				...
				default:
				   throw new ClassCastException("This method has not been " + "implemented for transferType " + TransferType_Renamed);
			}
		}
		...
		///            <code>transferType</code> is not supported by this
		///            color model </exception>
		public sealed override ColorModel CoerceData(WritableRaster raster, bool isAlphaPremultiplied)
		{
			if (!SupportsAlpha || this.AlphaPremultiplied == isAlphaPremultiplied)
			{
				return this;
			}
			...
			return new DirectColorModel(ColorSpace_Renamed, Pixel_bits, MaskArray[0], MaskArray[1], MaskArray[2], MaskArray[3], isAlphaPremultiplied, TransferType_Renamed);
		...
		public override bool IsCompatibleRaster(Raster raster)
		{
			...
			if (spsm.NumBands != NumComponents)
			{
				return false;
			}

			int[] bitMasks = spsm.BitMasks;
			for (int i = 0; i < NumComponents_Renamed; i++)
			{
		...
		{
			return "DirectColorModel: rmask=" + Red_mask.ToString("x") + " gmask=" + Green_mask.ToString("x") + " bmask=" + Blue_mask.ToString("x") + " amask=" + Alpha_mask.ToString("x");
		}
	}
